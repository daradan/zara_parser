...
class ZaraParser:
    ...
    def main(self):
        try:
            ...
            for section, values in categories.items():  # session -> woman, man, kid, beauty, origin
                for category in values:
                    for category_id, category_url in category.items():
                        url = self.make_urls(category_id)
                        data = self.get_data_from_json_loads(url, category_id)
                        print(data)
        except Exception as e:
            msg = f'ERROR: ZARA - {e}'
            send_to_telegram.main(msg, 'sendMessage', True)

    def get_categories(self) -> dict:
        categories_ids = dict()
        response = self.session.get(config.url_category, headers=config.headers)
    ...
    def make_urls(self, category_id: str) -> str:
        return f'{config.url_product}{category_id}/products'

    def get_data_from_json_loads(self, url: str, category_id: str) -> dict:
        response = self.session.get(url, params=config.params, headers=config.headers)
        json_loads = json.loads(response.text)
            ...
            for com_component in com_components:
                ...
                for color in colors:
                    ...
                    data = {'url': url,
                            ...
                            'image': ', '.join(self.make_photo_urls(color['xmedia'])),}
                    return data

    def make_url(self, seo: dict, category_id: str) -> str:
        if seo['keyword'] == '':
            return False
        keyword = f"{seo['keyword']}"
        seo_product_id = f"{seo['seoProductId']}"
        discern_product_id = f"{seo['discernProductId']}"
        url = f"{config.url}{keyword}-p{seo_product_id}.html?v1={discern_product_id}&v2={category_id}"
        return url

    def make_photo_urls(self, photos: list) -> list:
        photos_list = []
        for photo in photos:
            path = photo['path']
            name = photo['name']
            timestamp = photo['timestamp']
            photo_str = f"{config.url_photo}/{path}/w/750/{name}.jpg?ts={timestamp}"
...
#                         f"<a href='{self.data['url']}'>Купить на оф.сайте</a>\n\n" \
